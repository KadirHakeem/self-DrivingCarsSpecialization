...
class Bicycle(Bicycle):
    def step(self, v, w):
        ...
        self.xc += v * np.cos(theta + beta) * DT
        self.yc +=  v * np.sin(theta + beta) * DT
        self.theta += (v * np.cos(beta) * np.tan(delta) / L )* DT
        ...
        self.beta = np.arctan(lr * np.tan(delta) / L)
...
sample_time = 0.01
time_end = 3.75 + 3.75 + 3.75 + 3.75 + 3.75 + 3.75 + 3.75 + 3.75
model.reset()
...
v_data = np.zeros_like(t_data)
w_data = np.zeros_like(t_data)
delta=np.zeros_like(t_data)

# ==================================
#  Learner solution begins here
# ==================================
v_data[:] = 2*np.pi*16/30  
#w_data[:]= 0.15
delta[:]=np.arctan(2/8)
delta[377:1876]=-np.arctan(2/8)
w_data[:]=model.w_max
w_data[370:1815] = -model.w_max
for i in range(t_data.shape[0]):
    x_data[i] = model.xc
    y_data[i] = model.yc
