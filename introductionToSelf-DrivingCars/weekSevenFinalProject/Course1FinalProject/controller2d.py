...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            #5.3 ww=Vref/reff, we=ww/GR, Te=TL, we and Te > throttle
            #5.2 a_des = Kp * (vref - v) + ki* ((xref_prev + vref*dt)-(x_prev+v*dt)) + Kp* ((vref - v)- (vref_prev - v_prev))/(t-t_previous)
            #  vdes= vdes_prev + a_des*dt
            

            Kp=1
            Ki=1
            Kd=0.01
            x_previous = self.vars.x_previous
            t_previous = self.vars.t_previous
            v_desired_previous = self.vars.v_desired_previous
            v_previous = self.vars.v_previous
            DT= t-t_previous
            Dv = v_desired- v
            Dv_previous= v_desired_previous-v_previous   
            

            integral = self.vars.int_val + Dv*DT
            derivative = (Dv- Dv_previous)/DT


            a_des = Kp * Dv + Ki *integral + Kd * derivative
            #a_des = Kp * (v_desired- v) + Ki*((x_previous + v_desired*DT)-(x_previous + v*DT)) + Kd* ((v_desired- v)-(v_desired_previous- v_previous))/DT


            if a_des > 0:
                brake_output    = 0
                throttle_output= np.tanh(a_des)
            ...
            crosstrack_error = abs((a*x + b*y + c) / np.sqrt(a**2 + b**2))

            Ke=1;
            Ks=0;
            delta = heading_error + np.arctan(Ke * crosstrack_error/(Ks+v))
